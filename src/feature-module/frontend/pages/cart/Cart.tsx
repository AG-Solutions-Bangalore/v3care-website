...
const Cart = () => {
  ...
  const totalOriginalPrice = Object.values(groupedItems).reduce(
    (sum: number, group: any) => sum + group.originalTotal,
    0,
  );
  ...
  const validateForm = () => {
    ...
    for (const field of requiredFields) {
      if (!formData[field as keyof typeof formData]) {
        showNotification(
          `Please fill in the ${field.replace('order_', '').replace('_', ' ')} field`,
          'error',
        );
        return false;
      }
      ...
      const updatedCartItems = cartItems.map((item) => {
        ...
        if (priceResult) {
          const matchedPrice = priceResult.prices.find(
            (price: { service_price_for: string }) =>
    ...
    try {
      ...
      rzp.on(
        ...
        function (response: {
          ...
          showNotification(
            `Error:${errorMessage} , PaymentId:${response.error.metadata?.payment_id} , Payment_method: ${paymentMethod}`,
            'error',
            true,
          );
          // navigate('/booking-failed', {
                  ...
                  <div className="card-body">
                    {cartItems.length === 0 ? (
                      <div className="empty-cart">
                        ...
                        <p>
                          Looks like you haven&apos;t added any services to your
                          cart yet.
                        </p>
                        <button
